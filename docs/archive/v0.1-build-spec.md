# Archived: the v0.1.0 build specification

This is the original build specification for `ph-civic-data-mcp`, written on
2026-04-18 and used to build v0.1.0. It is kept for provenance only.

Large parts of it are now wrong. The package shipped through v0.6.0, the tool
count went from 11 to 32, PSA moved the poverty subject from `DB/1E/FY` to
`DB/1F/FY`, `fastmcp dev <file>` became `fastmcp dev inspector <file>`, and
`python -m build` became `uv build`. AQICN and its token requirement are gone;
air quality comes from Open-Meteo with no key.

Read `CLAUDE.md` for the current rules and `CHANGELOG.md` for what changed.
